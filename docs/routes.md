...
以下の処理はコード作成・ローカル検証済み。SQL 012はSupabase適用成功をユーザーが確認した。適用後はanon権限による公開カレンダー取得の表示形式・エラーなしをユーザーが確認済み。単一接続328項目（T09の110＋既存218）と別接続20ケース、検証用データの後片付けも、SQL 012適用後のSupabaseで成功をユーザーが確認した。2026年9月12日にIssue #78で職員画面を接続した。SQL 012のカレンダーはキャンプと利用停止を扱い、SQL 013以降で個人・団体も追加済み。
...
Issue #78の画面接続では、`/staff/calendar`をServer Componentの月・日取得へ接続し、GETによる閲覧だけでは更新Actionを呼ばない。月表示はキャンプ、地域活動個人、団体、利用停止を文字と配色で区別し、日別内訳から各正規詳細へ進む。キャンプ期間の合計行とキャンプ参加者の個別行は別表示とし、合計人数として足し合わせない。

利用停止の新規・編集・削除は`useActionState`を使い、失敗時に入力と許可済みの競合概要を保持する。削除には`confirmed=true`を追加し、理由と明示確認がそろわない操作をServer Actionでも拒否する。新しいSQLはなく、実職員での受入確認は未実施。
...
- 停止操作は申請IDだけをブラウザーから受け取り、Server Actionが申請者のUUIDと終了状態を再取得する。停止理由と確認を必須とし、完了後は同じ申請詳細へ戻す。
- 停止中または初期化中の本人には内部理由を表示せず、町への問い合わせ、初期化完了の連絡を待つこと、新規登録すること、以前の申請が自動で新アカウントへ結び付かないことを案内する。
...
本人要求は `begin_camp_application_pdf(application UUID, expected_input_version bigint, request_key UUID)`。成功はversion ID。同じ有効な要求の再送は同じID。stale-updateは最新の版へ自動差替えして再送しない。A3は接続済みで、配置や印字許可が無効なら拒否する。SQL036はA8設定ゲートを実装するが、検証済みOCI digestを登録するまでは `pdf-prerequisites-unavailable`、欄内上限を超える入力は `pdf-content-too-long` で拒否する。A9のフォーム・PDF表示確認・提出への接続は未実装。
