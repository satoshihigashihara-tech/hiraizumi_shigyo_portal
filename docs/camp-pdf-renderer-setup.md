...
変換はLinux上のLibreOffice Writerで行う。申請日と利用日は和暦に変換し、A3の印字用部屋名を使用する。氏名20、住所30、電話20、目的60、特記事項48、部屋名15文字をDBと変換器の両方で検査する。上限超過をフォント縮小で押し込まず拒否する。
...
insert into private.camp_pdf_render_setting_versions(
  ...
  '青木 幸保',20,30,20,30,60,48,15,clock_timestamp()
);
