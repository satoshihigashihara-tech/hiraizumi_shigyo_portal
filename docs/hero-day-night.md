...
- 0–0.3秒は朝、0.3–1.29秒で昼へ、1.29–1.59秒は昼、1.59–3秒で夜へ。夜は生成した画像の窓・玄関の明かりが現れ、3–3.3秒は夜を保持、3.3–4.5秒で朝へ戻り、4.5秒周期で繰り返す。一時停止・再生ボタン付き。ページ再訪・再読込では朝から始める。
...
- `app/components/ShareHouseHero.module.css`: 4.5秒周期のクロスフェード、静止設定、画像レイアウト。
...
- ループ版ではChromium・WebKitの幅1440/390/320pxで2周分の朝・昼・夜・夜→朝とループ境界、3枚のロード完了、横はみ出しなしを確認。一時停止・再生を実時間で確認。
- Chromium・WebKitでreduce設定時の夜静止を確認。
- 昼画像の読込を3.5秒保留し、朝のまま待機→読込後の再生を実時間で確認（初期の一回再生版）。
...
- 4.5秒周期の速度、夜の暗さ、スマートフォン実機での見え方を確認。
