---
name: fast-implement-issue
description: Fetch a GitHub issue, auto-generate plan, implement, and run fast local review/fix loop. Usage: /fast-implement-issue <issue番号> [--codex]
disable-model-invocation: true
---

# Fast Implement Issue: Issue → Investigate → Plan → Implement → Test → PR → Local Review & Fix Loop

Fetch a GitHub issue by number, analyze the codebase, auto-generate a plan, implement changes, then execute the full workflow with fast local review automatically.
Reviews are performed locally for speed, and the review history is posted to the PR as a single comment on completion.

## Arguments

- An issue number is **required** (e.g., `/fast-implement-issue 42`)
- `--codex` (optional): Use Codex MCP (`mcp__codex__codex`) for review. Defaults to Claude (local-code-reviewer) when omitted
- Example: `/fast-implement-issue 42 --codex`
- If no argument is provided, display the following message and abort:
  ```
  ⚠️ エラー: イシュー番号を指定してください。使い方: /fast-implement-issue <issue番号> [--codex]
  ```
...
### 9. Local Review & Fix Loop + Push + Post review history
...
#### 9a. Local Review & Fix Loop (max 3 iterations)
...
while loop_count < 3 AND lgtm == false:
  ...
  * If diff exceeds 5000 lines (Claude mode) or 8000 lines (Codex mode), display warning and break
  ...
  ## 9a-ii. Execute review (Claude or Codex)
  ...
  ## 9a-ii-save. Save review result to temp file
  mkdir -p working/tmp
...
#### 9c. Post review history as a PR comment
...
> このレビューは fast-implement-issue スキルによるローカルレビュー&修正ループの結果です。
...
### 10. Codex evaluation (conditional) + Completion message

#### Codex review evaluation (only when `--codex` AND LGTM not achieved)
...
3. **Display evaluation result**
   ...
   - Save the recommended action string for completion message
...
When `--codex` is specified, use `mcp__codex__codex` for review in Step 9a-ii.
...
- On `main` or `staging` branch → display error and abort
- Test failure (plan-executor agent reports `failed`) → abort in implemented state. Changes remain in the working tree with recovery instructions displayed to the user
