-- Fix lock statements and restrict consent metadata registration to the server.
...
drop function public.register_guardian_consent_document(
  uuid,
  text,
  text,
  integer
...
begin
  ...
  if not found or application_record.user_id <> expected_user_id then
    raise exception '申請が見つかりません。';
  end if;
